...
- Markdown 表示（見出し・リスト・表・引用・コードブロック・リンク）。コピーは元の Markdown を取得
- 履歴サイドバーから過去の会話を表示・再開。スマートフォンでは「設定・チャット履歴」ボタンで開閉
- Enter で送信、Shift + Enter で改行、日本語 IME に対応
...
Markdown は [Marked](https://marked.js.org/) で変換し、[DOMPurify](https://github.com/cure53/DOMPurify) で許可した要素・属性だけに制限します。スクリプト・イベント属性・危険なリンクは除去し、外部画像は読み込みません。依存ファイルはローカル配信します。
